...
@router.get("/sessions")
def list_sessions(current_user: models.User = Depends(get_current_user),
                  db: Session = Depends(get_db)):
    if current_user.role != "admin":
        raise HTTPException(status_code=status.HTTP_403_FORBIDDEN, detail="Admin access required")

    # query distinct session_ids with owner user_id (first user in session)
    rows = db.query(models.Message.session_id, models.Message.user_id).distinct(models.Message.session_id).all()
    result = []
